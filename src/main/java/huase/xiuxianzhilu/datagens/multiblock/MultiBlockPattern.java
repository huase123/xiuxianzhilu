...
/**
 * - @description:MultiBlockPattern类
 */
public class MultiBlockPattern {
    private final REBlockPredicate[][][] pattern;
    private final int depth;
    private final int height;
    private final int width;

    public MultiBlockPattern(REBlockPredicate[][][] pPattern) {
    ...
    @VisibleForTesting
    public REBlockPredicate[][][] getPattern() {
        return this.pattern;
    }

    @Nullable
    @VisibleForTesting
    public MultiBlockPattern.BlockPatternMatch matches(LevelReader pLevel, BlockPos pPos, Direction pFinger, Direction pThumb) {
     ...
     */
    @Nullable
    private MultiBlockPattern.BlockPatternMatch matches(BlockPos pPos, Direction pFinger, Direction pThumb, LoadingCache<BlockPos, BlockInWorld> pCache) {
...
//                    if (!this.pattern[k][j][i].test(pCache.getUnchecked(translateAndRotate(pPos, pFinger, pThumb, i, j, k)))) {
     ...
     */
    @Nullable
    public MultiBlockPattern.BlockPatternMatch find(LevelReader pLevel, BlockPos pPos) {
        LoadingCache<BlockPos, BlockInWorld> loadingcache = createLevelCache(pLevel, false);
            ...
            for(Direction direction : Direction.values()) {
                for(Direction direction1 : Direction.values()) {
                    if (direction1 != direction && direction1 != direction.getOpposite()) {
                        MultiBlockPattern.BlockPatternMatch blockpattern$blockpatternmatch = this.matches(blockpos, direction, direction1, loadingcache);
    ...
    public static LoadingCache<BlockPos, BlockInWorld> createLevelCache(LevelReader pLevel, boolean pForceLoad) {
        return CacheBuilder.newBuilder().build(new MultiBlockPattern.BlockCacheLoader(pLevel, pForceLoad));
    }
    ...
    public static class BlockPatternMatch {
        ...
        public BlockInWorld getBlock(int pPalmOffset, int pThumbOffset, int pFingerOffset) {
            return this.cache.getUnchecked(MultiBlockPattern.translateAndRotate(this.frontTopLeft, this.getForwards(), this.getUp(), pPalmOffset, pThumbOffset, pFingerOffset));
        }
