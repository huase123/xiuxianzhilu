    ...
    public void place(TreeDecorator.Context pContext) {
        ...
        if (!(randomsource.nextFloat() >= this.probability)) {
            ...
            list.stream().filter((blockPos) -> pContext.isAir(blockPos.below())).forEach(c->{
                if(!(randomsource.nextFloat() >= this.probability)){
                    pContext.setBlock(c, blockProvider.getState(randomsource,c.below()));
                }
            });
...
//                    return pContext.isAir(p_226022_) && pContext.isAir(p_226022_.relative(WORLDGEN_FACING));
//                }).findFirst();
//                if (!optional.isEmpty()) {
//                    pContext.setBlock(optional.get(), Blocks.BEE_NEST.defaultBlockState().setValue(BeehiveBlock.FACING, WORLDGEN_FACING));
//                    pContext.level().getBlockEntity(optional.get(), BlockEntityType.BEEHIVE).ifPresent((p_259007_) -> {
